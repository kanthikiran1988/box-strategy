...
ExpiryManager::getExpiries(bool includeWeekly, bool includeMonthly) {
    ...
    // Create a map to track the last expiry in each month
    std::map<std::pair<int, int>, std::chrono::system_clock::time_point> lastExpiryInMonth;
    
    // Identify the last expiry date for each month
    for (const auto& expiry : uniqueExpiries) {
        auto time_t_val = std::chrono::system_clock::to_time_t(expiry);
        std::tm* tm = std::localtime(&time_t_val);
        
        // Create a key for this month (year, month)
        std::pair<int, int> monthKey = {tm->tm_year, tm->tm_mon};
        
        // Check if we've seen this month before
        if (lastExpiryInMonth.find(monthKey) == lastExpiryInMonth.end()) {
            lastExpiryInMonth[monthKey] = expiry;
        } else {
            // If this expiry is later than what we've recorded, update it
            if (expiry > lastExpiryInMonth[monthKey]) {
                lastExpiryInMonth[monthKey] = expiry;
            }
        }
    }
    ...
    for (const auto& expiry : uniqueExpiries) {
        ...
        // Create a key for this month
        std::pair<int, int> monthKey = {tm->tm_year, tm->tm_mon};
        
        // Check if this is the last expiry in the month
        bool isMonthly = (expiry == lastExpiryInMonth[monthKey]);
